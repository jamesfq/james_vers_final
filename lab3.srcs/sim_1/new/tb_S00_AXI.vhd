...
port map (
      ...
      left_dds_data_o           => left_data,
      right_dds_data_o          => right_data,
    
      -- Debug ports to send to ILA
      left_dds_phase_inc_dbg_o  => left_phase,
      right_dds_phase_inc_dbg_o => right_phase,
...
----------------------------------------------------------------------------
-- Generate the AXI clock 
lrclk_gen_process : process
begin
	lrclk <= '0';				-- start low
	wait for LRCLK_PERIOD/2;		-- wait for half a clock period
	loop							-- toggle, and loop
	  lrclk <= not(lrclk);
	  wait for LRCLK_PERIOD/2;
	end loop;
end process lrclk_gen_process;
